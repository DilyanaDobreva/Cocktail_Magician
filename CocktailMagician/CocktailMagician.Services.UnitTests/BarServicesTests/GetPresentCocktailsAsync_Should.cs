...
{
    [TestClass]
    public class GetPresentCocktailsAsync_Should
    {
        [TestMethod]
        public async Task ThrowException_WhenIdIsInvalid()
        {
            ...
            using (var assertContext = new CocktailMagicianDb(options))
            {
                ...
                await Assert.ThrowsExceptionAsync<InvalidOperationException>(() => sut.GetPresentCocktailsAsync(invalidId));
            };
        }
        ...
        public async Task ThrowsExceptionWhen_BarIsDeleted()
        {
            ...
            using (var assertContext = new CocktailMagicianDb(options))
            {
                ...
                var sut = new BarServices(assertContext, barFactoryMock.Object, barCocktailFactoryMock.Object);
                await Assert.ThrowsExceptionAsync<InvalidOperationException>(() => sut.GetPresentCocktailsAsync(barId));
            };
        }
        [TestMethod]
        public async Task ReturnCocktailsOfferedInBar()
        {
            ...
            using (var assertContext = new CocktailMagicianDb(options))
            {
                ...
                var listOfCocktails = await sut.GetPresentCocktailsAsync(barId);

                Assert.AreEqual(1, listOfCocktails.Count());
                Assert.IsTrue(listOfCocktails.Any(c => c.Name == cocktail1TestName));
            }
        }
    }
}
